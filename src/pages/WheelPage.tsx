...
const WHEEL_COLORS = [
  '#00d4ff', '#f5a623', '#a855f7', '#10b981', '#ef4444',
...
function drawWheel(canvas: HTMLCanvasElement, items: string[], angleDeg: number) {
...
export function WheelPage() {
  ...
  const items = parseItems(inputText, dedup);
  ...
  const handleSpin = useCallback(() => {
    ...
    const endAngle = angleDegRef.current + extraRotations + targetOffset - (angleDegRef.current % 360);
  ...
  return (
    <div className="min-h-dvh pb-20 flex flex-col max-w-md mx-auto px-4 pt-4">
      {/* Header */}
      <div className="mb-4">
        <h1 className="text-xl font-extrabold" style={{ fontFamily: 'Exo 2, sans-serif' }}>Vòng quay tự do</h1>
        <p className="text-xs text-[#6b7f99]">Không tiêu chìa khóa · Nhập danh sách, quay ngẫu nhiên</p>
      </div>

      {/* Wheel */}
      <div className="relative flex items-center justify-center mb-4" style={{ minHeight: 300 }}>
        {/* Glow backdrop */}
        <div className="absolute inset-0 rounded-full pointer-events-none"
          style={{ background: spinning ? 'radial-gradient(circle, rgba(0,212,255,0.12) 0%, transparent 70%)' : 'transparent', transition: 'background 0.5s' }} />
        ...
        <div className="relative" style={{ width: 280, height: 280 }}>
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1 z-10">
            <svg viewBox="0 0 20 24" width={20} height={24}>
              <polygon points="10,0 20,20 10,14 0,20" fill="#f5a623" stroke="#080c18" strokeWidth="1.5" strokeLinejoin="round" />
            </svg>
          </div>
          ...
          {items.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center rounded-full"
              ...
              <p className="text-sm text-[#6b7f99] text-center px-8 leading-relaxed">
                Nhập danh sách<br />bên dưới để bắt đầu
              </p>
            </div>
          )}
        </div>
      ...
      {/* Result */}
      {result && (
        <div className="mb-4 p-4 rounded-2xl text-center animate-fade-in-up"
          style={{ background: 'rgba(0,212,255,0.08)', border: '1px solid rgba(0,212,255,0.35)', boxShadow: '0 0 20px rgba(0,212,255,0.1)' }}>
          <p className="text-xs text-[#6b7f99] uppercase tracking-widest mb-1">Kết quả</p>
          <p className="text-2xl font-extrabold" style={{ color: '#00d4ff', fontFamily: 'Exo 2, sans-serif' }}>{result}</p>
        </div>
      )}
      ...
      <button
        ...
        style={{
          ...
          background: canSpin ? 'linear-gradient(135deg, #00d4ff, #0088cc)' : 'rgba(107,127,153,0.15)',
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1.5">
          <label className="text-xs font-semibold text-[#6b7f99] uppercase tracking-widest">Danh sách</label>
          <span className="text-xs" style={{ color: items.length < 2 ? '#f87171' : '#4ade80' }}>
        ...
        <textarea
          ...
          style={{ background: 'rgba(14,22,40,0.8)', border: '1px solid rgba(0,212,255,0.15)', color: '#e8edf5', fontFamily: 'Be Vietnam Pro, sans-serif' }}
      ...
      <div className="flex items-center gap-4 mb-4 text-xs text-[#6b7f99]">
        <label className="flex items-center gap-1.5 cursor-pointer select-none">
          <input type="checkbox" checked={dedup} onChange={(e) => setDedup(e.target.checked)} className="accent-[#00d4ff]" />
          Loại trùng
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer select-none">
          <input type="checkbox" checked={removeWinner} onChange={(e) => setRemoveWinner(e.target.checked)} className="accent-[#00d4ff]" />
          Xóa mục vừa trúng
        </label>
        {inputText.trim() && (
          <button onClick={() => { setInputText(''); setResult(null); setHistory([]); }} className="ml-auto" style={{ color: '#6b7f99' }}>
            Xoá hết
          </button>
        )}
      </div>
      ...
      {history.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-[#6b7f99] uppercase tracking-widest mb-2">Lịch sử</p>
          <div className="flex flex-wrap gap-2">
            {history.map((h, i) => (
              <span key={i} className="px-2.5 py-1 rounded-full text-xs"
                style={{ background: 'rgba(14,22,40,0.8)', border: '1px solid rgba(107,127,153,0.2)', color: i === 0 ? '#00d4ff' : '#6b7f99' }}>
                {h}
              </span>
            ))}
          </div>
