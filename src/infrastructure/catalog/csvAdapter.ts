...
const RarityEnum = z.enum(["common", "rare", "epic", "diamond"])
...
const RawRowSchema = z.object({
  ...
  name_en: z.string().max(80).optional().default(""),
  description_en: z.string().max(240).optional().default(""),
  ...
  type: z.enum(["normal", "limited"]).optional(),
  ...
  base_weight: z.coerce.number().int().min(1).max(1000).optional(),
...
export function parseCsv(csvText: string): ParseResult {
  ...
  data.forEach((row, idx) => {
    ...
    if (!result.success) {
      result.error.issues.forEach((issue) => {
        errors.push(
          `Dòng ${lineNum}: ${issue.path.join(".")} — ${issue.message}`,
        )
      })
      return
    }
    const r = result.data
    if (seenIds.has(r.id)) {
      errors.push(`Dòng ${lineNum}: ID trùng "${r.id}"`)
      return
    }
    seenIds.add(r.id)
    dishes.push({
      ...
      categories: r.categories.split("|").map((value) => value.trim()).filter(Boolean),
      restaurantSearch: { queries: r.search_queries.split("|").map((value) => value.trim()).filter(Boolean).concat(r.search_query) },
      type: r.type,
      limitedEventId: r.limited_event_id || undefined,
      imageUrl: r.image_url || undefined,
      tags: r.tags,
      weight: r.weight,
      baseWeight: r.base_weight ?? r.weight,
      rarity: r.rarity,
  ...
  for (const slot of slots) {
    if (!dishes.some((d) => d.active && d.mealSlots.includes(slot))) {
      errors.push(
        `Banner "${slot}" không có món nào active. Catalog bị từ chối.`,
      )
    }
  }
