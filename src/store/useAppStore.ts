...
interface AppStore {
  ...
  init(): void;
  checkIn(): boolean;
  openChest(slot: MealSlot): { reward: RewardInstance; error: null } | { reward: null; error: string };
  fuse(inputIds: [string, string, string], targetSlot: MealSlot): { reward: RewardInstance; error: null } | { reward: null; error: string };
...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  fuse(inputIds, targetSlot) {
    ...
    if (err) return { reward: null, error: err };
    const pool = dishes.filter((d) => d.active && d.mealSlots.includes(targetSlot));
    if (pool.length === 0) return { reward: null, error: 'Không có món nào cho banner đích.' };
    const { state: newUser, reward } = applyFuse(user, dishes, inputIds, targetSlot);
    repository.saveUser(newUser);
    set({ user: newUser });
    return { reward, error: null };
  },
  ...
  toggleFavorite(rewardId) {
    ...
    const newUser = { ...user, rewards: newRewards, updatedAt: new Date().toISOString() };
    repository.saveUser(newUser);
    set({ user: newUser });
  },
  ...
  updatePreference(key, value) {
    const { user } = get();
    const newUser = { ...user, preferences: { ...user.preferences, [key]: value }, updatedAt: new Date().toISOString() };
    repository.saveUser(newUser);
    set({ user: newUser });
  },
  ...
  async loadRemoteCatalog(url) {
    ...
    try {
      ...
      if (result.dishes.length > 0) {
        const cache = { schemaVersion: 1 as const, sourceUrl: url, fetchedAt: new Date().toISOString(), hash: '', dishes: result.dishes };
